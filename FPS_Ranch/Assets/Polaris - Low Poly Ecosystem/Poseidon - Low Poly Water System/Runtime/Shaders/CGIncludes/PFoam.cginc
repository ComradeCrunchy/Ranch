...
void CalculateFoamColor(float sceneDepth, float surfaceDepth, float3 worldPos, float3 normal, float crestMask, out half4 foamColor)
{
	...
	half slopeFade = 1 - frac((worldPos.y + _Time.y * _SlopeFoamFlowSpeed) / _SlopeFoamDistance);
...
void CalculateFoamColorHQ(float sceneDepth, float surfaceDepth, float3 worldPos, float3 normal, float crestMask, out half4 foamColor)
{
	...
	half noiseBase = SampleFragmentNoise(worldPos.xz * noiseScale - noiseSpeed.xx * _Time.y * 0.2) * 0.5 + 0.5;
	half noiseFade = SampleFragmentNoise(worldPos.xz * noiseScale + noiseSpeed.xx * _Time.y) * 0.5 + 0.5;
