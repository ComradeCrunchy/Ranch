...
float4 CalculateRippleVertexOffset(float4 localPos, float rippleHeight, float rippleSpeed, float rippleScale, float3 flowDir)
{
	...
	float noiseBase = SampleVertexNoise((noisePos - flowDir.xz * _Time.y * rippleSpeed) * rippleScale) * 0.5 + 0.5;
	float noiseFade = lerp(0.5, 1, SampleVertexNoise((noisePos + flowDir.xz * _Time.y * rippleSpeed * 3) * rippleScale) * 0.5 + 0.5);
