...
namespace Pinwheel.Griffin
{
    ...
    public class GStylizedTerrain : MonoBehaviour
    {
            ...
            int meshResolution)
        {
            ...
            NativeArray<bool> dummyMarkers = new NativeArray<bool>(1, Allocator.TempJob);
            markers.Add(dummyMarkers);
            ...
            while (newVertexCreated && stitchSeamIteration <= stitchSeamMaxIteration)
            {
                ...
                for (int i = 0; i < chunksToUpdate.Count; ++i)
                {
                    ...
                    NativeArray<bool> leftMarkers = hasLeftMarkers ? leftChunk.GetVertexMarker() : dummyMarkers;
                    markers.Add(leftMarkers);

                    GTerrainChunk topChunk = GetTopNeighborChunk(c);
                    bool hasTopMarkers = topChunk != null;
                    NativeArray<bool> topMarkers = hasTopMarkers ? topChunk.GetVertexMarker() : dummyMarkers;
                    markers.Add(topMarkers);

                    GTerrainChunk rightChunk = GetRightNeighborChunk(c);
                    bool hasRightMarkers = rightChunk != null;
                    NativeArray<bool> rightMarkers = hasRightMarkers ? rightChunk.GetVertexMarker() : dummyMarkers;
                    markers.Add(rightMarkers);

                    GTerrainChunk bottomChunk = GetBottomNeighborChunk(c);
                    bool hasBottomMarkers = bottomChunk != null;
                    NativeArray<bool> bottomMarkers = hasBottomMarkers ? bottomChunk.GetVertexMarker() : dummyMarkers;
