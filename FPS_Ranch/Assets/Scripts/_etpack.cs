...
public class Jetpack : MonoBehaviour
{
    ...
    public bool isPlayergrounded() => _mPlayerCharacterCharacterController.isGrounded;
    ...
    void Start()
    {
        ...
        DebugUtility.HandleErrorIfNullGetComponent<PlayerCharacterController, Jetpack>(_mPlayerCharacterCharacterController, this, gameObject);

        _mPlayerInputHandler = GetComponent<PlayerInputHandler>();
        DebugUtility.HandleErrorIfNullGetComponent<PlayerInputHandler, Jetpack>(_mPlayerInputHandler, this, gameObject);
    ...
    void Update()
    {
        ...
        else if (!_mPlayerCharacterCharacterController.hasJumpedThisFrame && _mPlayerInputHandler.GetJumpInputDown())
        {
            m_CanUseJetpack = true;
        }

        // jetpack usage
        bool jetpackIsInUse = m_CanUseJetpack && isJetpackUnlocked  && currentFillRatio > 0f && _mPlayerInputHandler.GetJumpInputHeld();
        if(jetpackIsInUse)
        {
            ...
            // cancel out gravity
            totalAcceleration += _mPlayerCharacterCharacterController.gravityDownForce;

            if (_mPlayerCharacterCharacterController.characterVelocity.y < 0f)
            {
                // handle making the jetpack compensate for character's downward velocity with bonus acceleration
                totalAcceleration += ((-_mPlayerCharacterCharacterController.characterVelocity.y / Time.deltaTime) * jetpackDownwardVelocityCancelingFactor);
            }

            // apply the acceleration to character's velocity
            _mPlayerCharacterCharacterController.characterVelocity += Vector3.up * totalAcceleration * Time.deltaTime;
        ...
        else
        {
            ...
            {
                float refillRate = 1 / (_mPlayerCharacterCharacterController.isGrounded ? refillDurationGrounded : refillDurationInTheAir);
