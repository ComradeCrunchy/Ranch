...
[RequireComponent(typeof(PlayerInputHandler), typeof(PlayerAudio))]
...
public class PlayerCharacterController : MonoBehaviour
{
    ...
    void HandleCharacterMovement()
    {
        // horizontal character rotation
        {
            ...
            transform.Rotate(new Vector3(0f, (-_mPlayerInputHandler.GetLookInputsHorizontal() 
        ...
        // vertical camera rotation
        {
            ...
            m_CameraVerticalAngle += _mPlayerInputHandler.GetLookInputsVertical() * rotationSpeed * RotationMultiplier;
        ...
        bool isSprinting = _mPlayerInputHandler.GetSprintInputHeld();
        {
            if (isSprinting)
            {
            ...
            Vector3 worldspaceMoveInput = transform.TransformVector(_mPlayerInputHandler.GetMoveInput());
            ...
            if (isGrounded)
            {
                ...
                // jumping
                if (isGrounded && _mPlayerInputHandler.GetJumpInputDown())
                {
    ...
    private void HandleCharacterMovement()
    {
        ...
        {
            m_CameraVerticalAngle += _inputHandler.GetLookInputsVertical() * rotationSpeed * rotationMultiplier;
        ...
        if (Physics.CapsuleCast(capsuleBottomBeforeMove, capsuleTopBeforeMove, _controller.radius,
            characterVelocity.normalized, out RaycastHit hit,
            characterVelocity.magnitude * Time.deltaTime, ground, QueryTriggerInteraction.Ignore))
        {
    ...
    {
        float groundcheckRadius = isGrounded ? (_controller.skinWidth + groundCheckDistance) : k_GroundCheckDistanceInAir;
        ...
        if(Time.time >= m_lastTimeJumped+ k_JumpGroundingPreventionTime)
        {
            if (Physics.CapsuleCast(GetCapsuleBottomHemisphere(), GetCapsuleTopHemisphere(_controller.height),
                _controller.radius, Vector3.down, out RaycastHit hit, groundcheckRadius, ground,
                QueryTriggerInteraction.Ignore))
            {
                m_GroundNormal = hit.normal;
        ...
        else
        {
            if (!ignoreObstructions)
            {
                Collider[] standingOverlaps = Physics.OverlapCapsule(GetCapsuleBottomHemisphere(), 
                    GetCapsuleTopHemisphere(capsuleHeightStanding), groundCheckDistance,
